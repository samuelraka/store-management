...
cekKasir(); // kasir juga bisa input barang masuk
$user = userLogin();
        ...
        try {
            ...
            $stmt = $pdo->prepare(
                "INSERT INTO pembelian
                 (supplier_id, user_id, no_pembelian, tanggal_masuk, total_harga, keterangan)
                 VALUES (?, ?, ?, ?, ?, ?)"
            );
            $stmt->execute([
            ...
            foreach ($barang_ids as $i => $bid) {
                ...
                $stmt->execute([$pembelian_id, $bid, $qty, $harga, $subtotal]);
            ...
            $pdo->commit();
            redirect('index.php', 'Barang masuk berhasil dicatat! No: ' . $no_pembelian);
...
<head>
    ...
    <link rel="stylesheet" href="../../assets/css/admin/pembelian/tambah.css">
</head>
<body>
<aside class="sidebar">
                    ...
                    <div class="form-card">
                        ...
                        <div class="d-flex gap-2 mb-3">
                            ...
                            <button type="button" onclick="tambahBarang()"
                                    class="btn btn-outline-primary btn-sm" style="white-space:nowrap">
...
<script>
let itemCount = 0;
const barangData = <?= json_encode(array_column($barang_list, null, 'id')) ?>;

function tambahBarang() {
    const sel  = document.getElementById('pilihBarang');
    const opt  = sel.options[sel.selectedIndex];
    const bid  = sel.value;
    if (!bid) return alert('Pilih barang terlebih dahulu');

    // Cek duplikat
    if (document.querySelector(`[data-barang-id="${bid}"]`)) {
        return alert('Barang sudah ditambahkan');
    }
    ...
    div.innerHTML = `
        ...
        <div class="row g-2 align-items-center">
            <div class="col-12">
                ...
                <div style="font-size:.75rem;color:#6b7280">Stok saat ini: ${stok} ${satuan}</div>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <div class="subtotal-display"
                     style="font-weight:700;color:#16a34a;font-size:.9rem;padding-top:4px">
                    ${formatRp(harga)}
                </div>
            </div>
        </div>
